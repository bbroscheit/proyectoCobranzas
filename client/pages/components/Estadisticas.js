...
function Estadisticas({ clienteId }) {
  ...
  useEffect(() => {
    ...
    fetch(
      `http://${
        process.env.NEXT_PUBLIC_LOCALHOST
      }:3001/clientName?userId=${encodeURIComponent(
        userParse.id
      )}&clienteId=${encodeURIComponent(cliente)}`
    )
      .then((res) => res.json())
  ...
  const sortFacturas = (documentosSinRecibos, criteria) => {
    switch (criteria) {
      case "MontoAsc":
        return documentosSinRecibos.sort(
          (a, b) => a.montooriginal - b.montooriginal
        );
      case "MontoDesc":
        return documentosSinRecibos.sort(
          (a, b) => b.montooriginal - a.montooriginal
        );
      case "FechaDocumentoAsc":
        return documentosSinRecibos.sort(
          (a, b) => new Date(a.fechadocumento) - new Date(b.fechadocumento)
        );
      case "FechaDocumentoDesc":
        return documentosSinRecibos.sort(
          (a, b) => new Date(b.fechadocumento) - new Date(a.fechadocumento)
        );
      case "MontoPendiente":
        return documentosSinRecibos.sort(
  ...
  const currentFacturas = sortedFacturas.slice(
    indexOfFirstFactura,
    indexOfLastFactura
  );
  ...
  return (
    <div>
      <div className={styles.containerGrid}>
        <CardEstadisticasSaldos
          totalVencidas={totales.facturasVencidas}
          totalNoVencido={totales.facturasNoVencidas}
          recibosMes={totales.recibosMes}
        />
