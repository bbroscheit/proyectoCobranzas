...
import { sendLegales } from "../api/sendLegales";
import Swal from "sweetalert2";

function EnvioLegalesModal({ showModal, setShowModal, cliente }) {
  ...
  const onSubmit = (e) => {
    ...
    sendLegales({ numeroCliente: cliente, user, destinatario })
      .then((res) => {
        if (res.state === "success") {
          Swal.fire({ icon: "success", title: "Tu aviso de envío a legales fue realizado con éxito!", showConfirmButton: false, timer: 1000 });
        }
  ...
  return (
    <div className={styles.modal}>
      <div className={styles.modalContent}>
        <h2 className={styles.title}>¿ Deseas enviar un aviso de retiro y envío a legales?</h2>
...
export default EnvioLegalesModal;
