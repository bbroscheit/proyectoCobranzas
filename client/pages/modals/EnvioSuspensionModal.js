...
import { sendSuspension } from "../api/sendSuspension";
import Swal from "sweetalert2";

function EnvioSuspensionModal({ showModal, setShowModal, cliente }) {
  ...
  const onSubmit = (e) => {
    ...
    sendSuspension({ numeroCliente: cliente, user, destinatario })
      .then((res) => {
        if (res.state === "success") {
          Swal.fire({ icon: "success", title: "Tu aviso de suspensión fue realizado con éxito!", showConfirmButton: false, timer: 1000 });
        }
  ...
  return (
    <div className={styles.modal}>
      <div className={styles.modalContent}>
        <h2 className={styles.title}>¿ Deseas enviar un aviso de suspensión?</h2>
...
export default EnvioSuspensionModal;
