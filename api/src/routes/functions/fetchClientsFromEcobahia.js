...
const { CRM_USER, CRM_PASSWORD, CRM_SERVER, CRM_DATABASE_ECOBAHIA } = process.env;
console.log("Configuración CRM ecobahia:", {
  CRM_USER,
  CRM_PASSWORD,
  CRM_SERVER,
  CRM_DATABASE_ECOBAHIA,
});
...
const crmConfig = {
  ...
  server: CRM_SERVER,
  database: CRM_DATABASE_ECOBAHIA,
  options: {
    encrypt: true,
...
const fetchClientsFromCRMEcobahia = async () => {
  try {
    ...
    const existingClients = await Clientecobahia.findAll({
      where: {
  ...
  await Clientecobahia.bulkCreate(toCreate, {
...
module.exports = fetchClientsFromCRMEcobahia;